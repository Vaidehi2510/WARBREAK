...
try:
    ...
    try:
        from .llm_client import call_llm_json
    except Exception:  # pragma: no cover - FOGLINE can run heuristically.
        call_llm_json = None

try:
    from game_state import Assumption as GameAssumption
except Exception:  # pragma: no cover - package import path fallback.
    try:
        from .game_state import Assumption as GameAssumption
    except Exception:  # pragma: no cover - standalone FOGLINE tests do not need this.
        GameAssumption = None
...
def analyze_plan(request: FoglineAnalyzeRequest) -> FoglineAnalyzeResponse:
    ...
    if not _truthy_env("FOGLINE_DISABLE_LLM"):
        llm_candidates, llm_warning = _extract_with_llm(request, catalog)
        if llm_warning:
        ...
        warnings.append("LLM extraction skipped because FOGLINE_DISABLE_LLM is set.")
    ...
    if compiler_mode == "fallback" and not any("LLM" in warning for warning in warnings):
        warnings.append("LLM extraction unavailable; used deterministic fallback heuristics.")
...
def extract_assumptions(plan: str) -> list[Any]:
    """Compatibility adapter for the WARBREAK game engine.

    The FOGLINE compiler returns rich UI/game seeds. The existing turn engine
    expects compact GameState assumptions, so this adapter preserves the richer
    compiler while keeping the rest of the current backend contract stable.
    """
    if GameAssumption is None:
        raise RuntimeError("GameState Assumption model is unavailable.")

    response = analyze_plan(
        FoglineAnalyzeRequest(
            plan_text=plan,
            scenario_name="WARBREAK Mission Plan",
    ...
    for item in response.assumptions:
        immediate_effects = item.break_event_template.immediate_effects or []
        cascade_effect = (
            immediate_effects[0]
            if immediate_effects
            else item.break_event_template.sitrep
        ...
        assumptions.append(
            GameAssumption(
                ...
                criticality=_clamp_float(item.score_breakdown.consequence_severity, 0, 1),
                fragility=int(round(item.fragility_score)),
                basis=item.why_this_pattern_matches,
                doctrine_ref=item.pattern_id,
                dependencies=outgoing.get(item.id, []),
...
def _extract_with_llm(
    ...
    if call_llm_json is None:
        return [], "LLM provider chain unavailable; used deterministic fallback heuristics."
    ...
    try:
        prompt = (
            ...
            "Return only a JSON object matching return_shape."
        )
        text = call_llm_json(prompt, max_tokens=2400, temperature=0.1)
        ...
        candidates = [_coerce_candidate(item, request, catalog, "llm") for item in raw_items]
        model_warnings = [
        ...
        return [], f"LLM extraction failed; used deterministic fallback heuristics. Reason: {type(exc).__name__}."
